...
{
    public class EndToEndBrightChainTest : IClassFixture<EndToEndBrightChainTest.BrightChainFixture>
    {
        private const string DatabaseName = "BrightChainEndToEndTest";
        ...
        public void Find_with_empty_resource_id_throws()
        {
            ...
            using (var context = new PartitionKeyContextWithResourceId(options))
            {
                ...
                Assert.Equal(
                    BrightChainStrings.InvalidResourceId,
...
OFFSET 0 LIMIT 1");
        ...
        public async Task Add_update_delete_query_throws_if_no_container()
        {
            ...
            using (var context = new CustomerContext(options))
            {
                ...
                Assert.StartsWith(
                    ...
                    (await Assert.ThrowsAsync<BrightChainException>(() => context.SaveChangesAsync())).Message);
            }
            ...
            using (var context = new CustomerContext(options))
            {
                Assert.StartsWith(
                    "Response status code does not indicate success: NotFound (404); Substatus: 0",
                    (await Assert.ThrowsAsync<BrightChainException>(() => context.Set<Customer>().SingleAsync())).Message);
            }
        }
        ...
        public class BrightChainFixture : ServiceProviderFixtureBase, IAsyncLifetime
        {
            ...
            public virtual BrightChainTestStore TestStore { get; }
