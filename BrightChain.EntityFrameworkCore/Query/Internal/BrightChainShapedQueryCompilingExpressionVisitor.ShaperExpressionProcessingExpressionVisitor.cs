...
namespace BrightChain.EntityFrameworkCore.Query.Internal
{
    public partial class BrightChainShapedQueryCompilingExpressionVisitor
    {
        private sealed class ShaperExpressionProcessingExpressionVisitor : ExpressionVisitor
        {
            ...
            private readonly BrightChainShapedQueryCompilingExpressionVisitor _brightChainShapedQueryCompilingExpressionVisitor;
            ...
            public ShaperExpressionProcessingExpressionVisitor(
                BrightChainShapedQueryCompilingExpressionVisitor brightChainShapedQueryCompilingExpressionVisitor,
                BrightChainQueryExpression brightChainQueryExpression,
                bool tracking)
            {
                this._brightChainShapedQueryCompilingExpressionVisitor = brightChainShapedQueryCompilingExpressionVisitor;
                this._valueBufferParameter = brightChainQueryExpression.CurrentParameter;
            ...
            public LambdaExpression ProcessShaper(Expression shaperExpression)
            {
                ...
                result = Expression.Block(this._variables, this._expressions);

                if (this._valueBufferParameter == null)
                {
                    // If parameter is null then the projection is not really server correlated so we can just put anything.
                    this._valueBufferParameter = Expression.Parameter(typeof(ValueBuffer));
                }

                return Expression.Lambda(result, QueryCompilationContext.QueryContextParameter, this._valueBufferParameter);
            }
                    ...
                    case EntityShaperExpression entityShaperExpression:
                        {
                            ...
                            if (!this._mapping.TryGetValue(key, out var variable))
                            {
                                ...
                                var innerShaper = this._brightChainShapedQueryCompilingExpressionVisitor.InjectEntityMaterializers(entityShaperExpression);
                                innerShaper = this.Visit(innerShaper);
                                this._expressions.Add(Expression.Assign(variable, innerShaper));
                    ...
                    case ProjectionBindingExpression projectionBindingExpression:
                        {
                            ...
                            if (!this._mapping.TryGetValue(key, out var variable))
                            {
                                ...
                                this._variables.Add(variable);
                                var queryExpression = (BrightChainQueryExpression)projectionBindingExpression.QueryExpression;
                                if (this._valueBufferParameter == null)
                                {
                                ...
                                this._expressions.Add(Expression.Assign(
                    ...
                    case IncludeExpression includeExpression:
                        {
                            ...
                            if (includeExpression.Navigation.IsCollection)
                            {
                                ...
                                var shaperLambda = new ShaperExpressionProcessingExpressionVisitor(this._brightChainShapedQueryCompilingExpressionVisitor, this._tracking)
                                ...
                                Expression.Call(
                                    ...
                                    Expression.Constant(this._tracking),
...
#pragma warning restore EF1001 // Internal EF Core API usage.
                            ...
                            {
                                this._expressions.Add(Expression.Call(
