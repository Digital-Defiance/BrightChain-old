...
namespace BrightChain.EntityFrameworkCore.Query.Internal
{
    ...
    /// </summary>
    public class BrightChainExpressionTranslatingExpressionVisitor : ExpressionVisitor
    {
        ...
        private static readonly MethodInfo _parameterValueExtractor =
            typeof(BrightChainExpressionTranslatingExpressionVisitor).GetRequiredDeclaredMethod(nameof(ParameterValueExtractor));

        private static readonly MethodInfo _parameterListValueExtractor =
            typeof(BrightChainExpressionTranslatingExpressionVisitor).GetRequiredDeclaredMethod(nameof(ParameterListValueExtractor));

        private static readonly MethodInfo _getParameterValueMethodInfo =
            typeof(BrightChainExpressionTranslatingExpressionVisitor).GetRequiredDeclaredMethod(nameof(GetParameterValue));
        ...
        private static readonly MethodInfo _BrightChainLikeMethodInfo =
            typeof(BrightChainExpressionTranslatingExpressionVisitor).GetRequiredDeclaredMethod(nameof(BrightChainLike));
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public BrightChainExpressionTranslatingExpressionVisitor(
            ...
            QueryableMethodTranslatingExpressionVisitor queryableMethodTranslatingExpressionVisitor)
        {
            ...
            this._entityReferenceFindingExpressionVisitor = new EntityReferenceFindingExpressionVisitor();
        ...
        private Expression? TranslateInternal(Expression expression)
        {
            ...
            return result == QueryCompilationContext.NotTranslatedExpression
                || this._entityReferenceFindingExpressionVisitor.Find(result)
                ? null
                : result;
        }
        ...
        protected override Expression VisitBinary(BinaryExpression binaryExpression)
        {
            ...
            {
                return this.Visit(ConvertObjectArrayEqualityComparison(binaryExpression.Left, binaryExpression.Right));
            }

            var newLeft = this.Visit(binaryExpression.Left);
            var newRight = this.Visit(binaryExpression.Right);
            ...
            if (binaryExpression.NodeType == ExpressionType.Equal
                || binaryExpression.NodeType == ExpressionType.NotEqual)
            {
                var property = this.FindProperty(newLeft) ?? this.FindProperty(newRight);
        ...
        protected override Expression VisitExtension(Expression extensionExpression)
        {
            ...
            switch (extensionExpression)
            {
                ...
                case ProjectionBindingExpression projectionBindingExpression
                    ...
                    return ((BrightChainQueryExpression)projectionBindingExpression.QueryExpression).GetProjection(projectionBindingExpression);

                case BrightChainGroupByShaperExpression BrightChainGroupByShaperExpression:
                    return new GroupingElementExpression(
                        BrightChainGroupByShaperExpression.GroupingParameter,
                        BrightChainGroupByShaperExpression.ElementSelector,
                        BrightChainGroupByShaperExpression.ValueBufferParameter);
        ...
        protected override Expression VisitMember(MemberExpression memberExpression)
        {
            ...
            if (this.TryBindMember(innerExpression, MemberIdentity.Create(memberExpression.Member), memberExpression.Type) is Expression result)
            {
                return result;
            }
        ...
        {
            var expression = this.Visit(memberAssignment.Expression);
        ...
        protected override Expression VisitMethodCall(MethodCallExpression methodCallExpression)
        {
            ...
            {
                return this.TryBindMember(this.Visit(source), MemberIdentity.Create(propertyName), methodCallExpression.Type)
                    ?? throw new InvalidOperationException(CoreStrings.QueryUnableToTranslateEFProperty(methodCallExpression.Print()));
            }

            // EF Indexer property
            if (methodCallExpression.TryGetIndexerArguments(this._model, out source, out propertyName))
            {
                return this.TryBindMember(this.Visit(source), MemberIdentity.Create(propertyName), methodCallExpression.Type)
                    ?? QueryCompilationContext.NotTranslatedExpression;
            }
                ...
                && methodCallExpression.Arguments.Count > 0)
            {
                if (methodCallExpression.Arguments[0].Type.TryGetElementType(typeof(IQueryable<>)) == null
                    && this.Visit(methodCallExpression.Arguments[0]) is GroupingElementExpression groupingElementExpression)
                {
                    ...
                    {
                        var predicate = this.TranslateInternal(RemapLambda(groupingElement, lambdaExpression));
                    ...
                    {
                        var selector = this.TranslateInternal(groupingElement.Selector);
            ...
            // Subquery case
            var subqueryTranslation = this._queryableMethodTranslatingExpressionVisitor.TranslateSubquery(methodCallExpression);
            if (subqueryTranslation != null)
            {
                var subquery = (BrightChainQueryExpression)subqueryTranslation.QueryExpression;
                ...
                || methodCallExpression.Method == _likeMethodInfoWithEscape)
            {
                ...
                {
                    var argument = this.Visit(methodCallExpression.Arguments[i]);
                ...
                return Expression.Call(_BrightChainLikeMethodInfo, visitedArguments);
            }
                ...
                && methodCallExpression.Arguments.Count == 1)
            {
                ...
                var right = this.Visit(methodCallExpression.Arguments[0])!;
                ...
                && methodCallExpression.Arguments.Count == 2)
            {
                ...
                var left = this.Visit(methodCallExpression.Arguments[0])!;
                var right = this.Visit(methodCallExpression.Arguments[1])!;
                ...
                && method.GetGenericMethodDefinition().Equals(EnumerableMethods.Contains))
            {
                ...
                if (this.TryRewriteContainsEntity(enumerable,
            ...
            {
                @object = this.Visit(methodCallExpression.Object);
        ...
        protected override Expression VisitNew(NewExpression newExpression)
        {
            ...
            {
                var newArgument = this.Visit(argument);
        ...
        protected override Expression VisitNewArray(NewArrayExpression newArrayExpression)
        {
            ...
            {
                var newExpression = this.Visit(expression);
                if (newExpression == QueryCompilationContext.NotTranslatedExpression)
                {
                    return QueryCompilationContext.NotTranslatedExpression;
                }
        ...
        protected override Expression VisitTypeBinary(TypeBinaryExpression typeBinaryExpression)
        {
            ...
            if (typeBinaryExpression.NodeType == ExpressionType.TypeIs
                && this.Visit(typeBinaryExpression.Expression) is EntityReferenceExpression entityReferenceExpression)
            {
                ...
                if (derivedType != null)
                {
                    ...
                    var boundProperty = this.BindProperty(entityReferenceExpression, discriminatorProperty, discriminatorProperty.ClrType);
        ...
        protected override Expression VisitUnary(UnaryExpression unaryExpression)
        {
            ...
            var newOperand = this.Visit(unaryExpression.Operand);
        ...
        private Expression? TryBindMember(Expression? source, MemberIdentity member, Type type)
        {
            ...
            if (property != null)
            {
                return this.BindProperty(entityReferenceExpression, property, type);
            }
        ...
        private Expression? BindProperty(EntityReferenceExpression entityReferenceExpression, IProperty property, Type type)
        {
            ...
            {
                var valueBufferExpression = this.Visit(entityReferenceExpression.ParameterEntity.ValueBufferExpression);
            ...
            if (entityReferenceExpression.SubqueryEntity != null)
            {
                ...
                var BrightChainQueryExpression = (BrightChainQueryExpression)entityReferenceExpression.SubqueryEntity.QueryExpression;
                ...
                var entityProjectionExpression = (EntityProjectionExpression)BrightChainQueryExpression.GetProjection(
                    projectionBindingExpression);
                readValueExpression = entityProjectionExpression.BindProperty(property);

                return ProcessSingleResultScalar(
                    BrightChainQueryExpression,
                    readValueExpression,
                    type);
            }
        ...
        private static Expression ProcessSingleResultScalar(
            BrightChainQueryExpression BrightChainQueryExpression,
            Expression readValueExpression,
            Type type)
        {
            if (BrightChainQueryExpression.ServerQueryExpression is not NewExpression)
            {
        ...
        private Expression CreatePropertyAccessExpression(Expression target, IProperty property)
        {
            switch (target)
            {
                ...
                case NewExpression newExpression
                    ...
                    return this.CreatePropertyAccessExpression(GetValue(newExpression), property);

                case MemberInitExpression memberInitExpression
                    when CanEvaluate(memberInitExpression):
                    return this.CreatePropertyAccessExpression(GetValue(memberInitExpression), property);
...
#pragma warning restore IDE0066 // Convert switch statement to expression
        ...
        private static bool BrightChainLike(string matchExpression, string pattern, string escapeCharacter)
        {
        ...
        private sealed class EntityReferenceExpression : Expression
        {
            ...
            {
                this.SubqueryEntity = subquery;
                this.EntityType = ((EntityShaperExpression)subquery.ShaperExpression).EntityType;
            }

            private EntityReferenceExpression(EntityReferenceExpression entityReferenceExpression, IEntityType entityType)
            {
                this.ParameterEntity = entityReferenceExpression.ParameterEntity;
                this.SubqueryEntity = entityReferenceExpression.SubqueryEntity;
            ...
            public Expression Convert(Type type)
            {
                if (type == typeof(object) // Ignore object conversion
                    || type.IsAssignableFrom(this.Type)) // Ignore casting to base type/interface
                {
                    return this;
                }

                var derivedEntityType = this.EntityType.GetDerivedTypes().FirstOrDefault(et => et.ClrType == type);
        ...
        private sealed class GroupingElementExpression : Expression
        {
            ...
            {
                this.Source = source;
                this.ValueBufferParameter = valueBufferParameter;
            ...
            public override Type Type
                => typeof(IEnumerable<>).MakeGenericType(this.Selector.Type);
