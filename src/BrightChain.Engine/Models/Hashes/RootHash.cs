...
namespace BrightChain.Engine.Models.Blocks
{
    ...
    public class RootHash : BlockHash, IDataHash, IComparable<RootHash>, IEquatable<RootHash>
    {
        ...
        public RootHash(BlockSize blockSize)
            : base(typeof(RootBlock), blockSize, EmptyHashBytes(), false)
        {
        }
        ...
        public bool Equals(RootHash other)
        {
            return !(other is null)
                ? other.SourceDataLength == this.SourceDataLength &&
                  ReadOnlyMemoryComparer<byte>.Compare(this.HashBytes, other.HashBytes) == 0
                : false;
        }

        private static ReadOnlyMemory<byte> EmptyHashBytes()
        {
            var emptyHashBytes = new byte[HashSizeBytes];
