    ...
    where T : new()
{
    ...
    public static IEnumerable<ChainLinqObjectBlock<T>> SetNextLinks(IEnumerable<ChainLinqObjectBlock<T>> blocks)
    {
        ...
        {
            var block = blocks.ElementAt(index: i);
            var previousBlock = blocks.ElementAt(index: i - 1);
    ...
    {
        var blocks = new List<ChainLinqObjectBlock<T>>();
        foreach (var o in objects)
        {
            blocks.Add(item: ChainLinqObjectBlock<T>.MakeBlock(
                blockParams: blockParams,
                blockObject: o));
        }

        return new ChainLinq<T>(blocks: blocks);
    }
    ...
    {
        return this.ObjectBlocks.Select(selector: b => b.BlockObject);
    }
    ...
    ///     Do NOT rely on the Id of these blocks for anything other than comparison of whether the contents have changed, prior to being
    ///     brightened into a BrightChain.
    /// </summary>
    /// <returns></returns>
    public IEnumerable<BlockHash> Hashes()
    {
        return this.ObjectBlocks.Select(selector: b => b.Id);
    }
    ...
    {
        return await BrightenAllAsync(brightBlockService: brightBlockService,
                objectBlocks: this.ObjectBlocksAsync())
            .ConfigureAwait(continueOnCapturedContext: false);
    }

    public static async Task<BrightChain> BrightenAllAsync(BrightBlockService brightBlockService,
        IAsyncEnumerable<ChainLinqObjectBlock<T>> objectBlocks)
    {
        using (var sha = SHA256.Create())
        {
            var received = 0;
            var expected = await objectBlocks.CountAsync().ConfigureAwait(continueOnCapturedContext: false);
