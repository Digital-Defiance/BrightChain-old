...
public class BlockBrightenerService
{
    ...
    /// <summary>
    ///     Brightening is the process of XORing to correlate the data with block of random data and make it appear more random or "white" as white
    ///     light is broad multi-spectrum light.
    /// </summary>
    /// <param name="identifiableBlock"></param>
    /// <returns></returns>
    public BrightenedBlock Brighten(IdentifiableBlock identifiableBlock, out BrightenedBlock[] randomizersUsed,
        out TupleStripe brightenedStripe)
    {
        ...
        for (var i = 0; i < randomizersUsed.Length; i++)
        {
            ...
            this.resultCache.Set(value: randomizersUsed[i]);
        }
        ...
        var brightBlock = new BrightenedBlock(
            ...
            data: identifiableBlock.XOR(others: randomizersUsed),
            constituentBlockHashes: randomizersUsed.Select(selector: b => b.Id).ToArray());
