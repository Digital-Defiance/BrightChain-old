...
namespace BrightChain.EntityFrameworkCore.Metadata.Conventions
{
    /// <summary>
    ///     <para>
    ///         A convention that adds the 'id' property - a key required by Azure BrightChain.
    ///     </para>
    ///     <para>
        ...
        IEntityTypeBaseTypeChangedConvention
    {
        ...
        private static void ProcessJObjectProperty(IConventionEntityTypeBuilder entityTypeBuilder)
        {
            ...
            if (entityType.BaseType == null
                && !entityType.IsKeyless)
            {
                var jObjectProperty = entityTypeBuilder.Property(typeof(JsonElement), JObjectPropertyName);
        ...
        {
            if (name == BrightChainAnnotationNames.ContainerName
                && (annotation?.Value == null
                    || oldAnnotation?.Value == null))
            {
                ProcessIdProperty(entityTypeBuilder);
            }
            else if (name == BrightChainAnnotationNames.PartitionKeyName)
            {
        ...
        public virtual void ProcessPropertyAnnotationChanged(
            ...
            IConventionContext<IConventionAnnotation> context)
        {
            if (name == BrightChainAnnotationNames.PropertyName
