...
namespace BrightChain.EntityFrameworkCore.Query.Internal
{
    ...
    /// </summary>
    public class BrightChainSqlTranslatingExpressionVisitor : ExpressionVisitor
    {
        ...
        private static readonly MethodInfo _parameterValueExtractor =
            typeof(BrightChainSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterValueExtractor));

        private static readonly MethodInfo _parameterListValueExtractor =
            typeof(BrightChainSqlTranslatingExpressionVisitor).GetTypeInfo().GetDeclaredMethod(nameof(ParameterListValueExtractor));
        ...
        ///     doing so can result in application failures when updating to a new Entity Framework Core release.
        /// </summary>
        public BrightChainSqlTranslatingExpressionVisitor(
        ...
        {
            return new SqlConstantExpression(Check.NotNull(constantExpression, nameof(constantExpression)), null);
        }
        ...
        {
            return parameterExpression.Name?.StartsWith(QueryCompilationContext.QueryParameterPrefix, StringComparison.Ordinal) == true
        ...
        {
            return expression is SqlConstantExpression sqlConstant && sqlConstant.Value == null;
        }

        private SqlConstantExpression GetConstantOrNull(Expression expression)
        {
            return CanEvaluate(expression)
...
#pragma warning restore IDE0066 // Convert switch statement to expression
        ...
        private sealed class SqlTypeMappingVerifyingExpressionVisitor : ExpressionVisitor
        {
            protected override Expression VisitExtension(Expression extensionExpression)
            {
                ...
                {
                    throw new InvalidOperationException(BrightChainStrings.NullTypeMappingInSqlTree(sqlExpression.Print()));
                }
